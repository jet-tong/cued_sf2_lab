...
import numpy as np
from cued_sf2_lab.laplacian_pyramid import quant3, quant4
...
def lbtenc(X: np.ndarray, qstep: float, N: int = 8, M: int = 8,
    ...
    Yq = quant3(Yr, qstep, qstep).astype('int')
...
def lbtdec(vlc: np.ndarray, qstep: float, N: int = 8, M: int = 8,
    ...
    Zi = quant4(Zq, qstep, qstep)
    m, n = Zi.shape
    N_m, N_n = N, N
    Zi = Zi.reshape(N_m, m // N_m, N_n, n // N_n)  # subdivide the axes
    Zi = Zi.transpose((1, 0, 3, 2))                # permute them
    Zi = Zi.reshape(m, n) 
...
def enc2bits(X: np.ndarray, qstep: float, bits: int, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True, s: float = 1.0
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
    ...
    def objective(qstep):
        if log:
            print('Quantising to step size of {}'.format(qstep))
        Yq = quant3(Y, qstep, qstep).astype('int')
        ...
        vlc = np.concatenate([np.zeros((0, 2), dtype=np.intp)] + vlc)
        b = vlc[:,1].sum()      
        return np.abs(b - bits)
    results = minimize(objective, 20, method='Nelder-Mead')
    qstep = results.x[0]
